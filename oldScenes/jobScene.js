class JobScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.tilemapTiledJSON('jobmap', 'assets/images/tilemaps/jobtilemap.json');
        this.load.image('jobtiles', 'assets/images/tilemaps/tileset.png');
    }
    ...
    create() {
        ...
        for (let i = 0; i < this.sceneTriggers.length; i++) {
            this.physics.add.overlap(this.character, this.sceneTriggers[i].trigger, function () {
                this.scene.start(this.sceneTriggers[i].scene, {from: 'JobScene'});
            }, null, this);
        }
    ...
    buildMap() {
        let map = this.make.tilemap({ key: 'jobmap' });
        let tileset = map.addTilesetImage('tileset', 'jobtiles');
        this.mapData = {
            map: map,
            tileset: tileset,
